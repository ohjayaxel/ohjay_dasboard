...
begin
  ...
  --   name: google_ads_sync_function_key
  --   value: <YOUR_SUPABASE_SERVICE_ROLE_JWT>
  perform vault.update_secret('google_ads_sync_function_key', '<SET_IN_SUPABASE_VAULT>');
exception
  when others then
    -- If update fails, create new
    perform vault.create_secret('<SET_IN_SUPABASE_VAULT>', 'google_ads_sync_function_key');
end
$$;
